...
async def create_studio_agent(request: StudioAgentRequest):
    ...
    try:
        ...
        return {
            ...
            "agent_id": agent_id,
            "agent": agent,
            "message": "Studio agent created successfully"
        }
...
async def get_agent_config(agent_id: str):
    """Get detailed agent configuration for editing"""
    agent = agent_service.get_agent(agent_id)
    if not agent:
    ...
    return {
        ...
        "agent": agent,
        "editable_fields": [
            "name", "voice_profile", "personality", "response_style",
            "capabilities", "custom_prompts"
        ],
...
async def update_agent_config(agent_id: str, config_updates: Dict[str, Any]):
    ...
    try:
        ...
        # Note: This would require implementing config update in agent_service
...
async def test_agent_conversation(agent_id: str, request: TestConversationRequest):
    """Test agent with a sequence of messages"""
    try:
        results = []
        ...
        for i, message in enumerate(request.test_messages):
            ...
            results.append({
                ...
                "agent_response": response["text_response"],
        ...
        return {
            ...
            "agent_id": agent_id,
            "message": "Agent testing completed successfully"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def deploy_to_test_room(agent_id: str):
    """Deploy agent to a test room for live testing"""
    try:
        # Create a test room
        ...
        # Generate token for tester
        test_token = await livekit_service.generate_token(
            room_name=test_room_name,
        ...
        return {
            ...
            "agent_id": agent_id,
            "message": "Agent deployed to test room successfully"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_advanced_templates():
    """Get advanced agent templates with detailed configurations"""
    templates = agent_service.get_agent_templates()
    
    # Add advanced template information
    ...
    return {
        "success": True,
        "templates": templates,
        "message": "Advanced templates retrieved successfully"
    }
...
async def get_system_status():
    """Get overall system status for Studio dashboard"""
    try:
        # Get service status
        ...
        return {
            ...
            "system_status": {
                ...
                "livekit_service": "active" if livekit_configured else "limited",
                "active_agents": ai_active_sessions,
